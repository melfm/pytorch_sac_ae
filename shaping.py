...
class ImgGANShaping(Shaping):
    def __init__(self, dims, max_u, gamma, layer_sizes,
                 potential_weight, norm_eps,
                 norm_clip, **kwargs):
    ...
    def train(self, batch):
        ...
        eta = (
            torch.FloatTensor(
                real_images_grad.shape[0], *([1] * (len(real_images_grad.shape) - 1)))
            .uniform_(0, 1)
            .to(device)
        )
